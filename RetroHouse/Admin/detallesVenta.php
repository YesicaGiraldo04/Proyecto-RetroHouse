...
<body>
    <?php
        ...
        if(isset($_SESSION['user_rol']) && $_SESSION['user_rol'] == 1){
            $idVenta = htmlentities($_POST['idVenta']);
            $consulta = $conn->prepare("SELECT detalleventa.idDetalleVenta, detalleventa.cantidad, detalleventa.precioUnitario, detalleventa.idVenta, producto.nombreProducto FROM detalleventa INNER JOIN producto ON detalleventa.idProducto = producto.idProducto WHERE idVenta = :idVenta");
            $consulta->execute([":idVenta" => $idVenta]);
                        ...
                        <div class="tablas p-3">
                            <div class="row">
                                <div class="col d-grid gap-2">
                                    <button class="btn btn-secondary my-3" onclick="window.location='ventasAdmin.php'">Regresar</button>
                                </div>
                            </div>
                                ...
                                <tbody>
                                    <?php
                                        foreach($compras as $x => $x_value){
                                            echo "
                                            <tr>
                                                <td>".$x_value['idDetalleVenta']."</td>
                                                <td>".$x_value['idVenta']."</td>
