<?php
    ...
    if (isset($_POST['idProducto']) && !empty($_POST['idProducto'])){
        $eliminar = $conn->prepare("UPDATE producto SET estado = 0 WHERE idProducto = :idProducto ;");
        ...
        $eliminar = $conn->prepare("UPDATE proveedor SET estado = 0 WHERE idProveedor = :idProveedor ;");
        $eliminar->bindparam(':idProveedor',$_POST['idProveedor']);
        $resultado = $eliminar->execute();
        if ($resultado){
            echo '
                <script>
                    alert("Proveedor deshabilitado exitosamente");
