...
<body>
    <?php
        ...
        if(isset($_SESSION['user_rol']) && $_SESSION['user_rol'] == 1){
            $consulta = $conn->prepare("SELECT compra.*, proveedor.nombreProveedor FROM compra INNER JOIN proveedor ON compra.idProveedor = proveedor.idProveedor");
            $consulta->execute();
                        ...
                        <div class="tablas p-3">
                            <div class="row">
                                <div class="col d-grid gap-2">
                                    <button class="btn btn-success my-3" onclick="window.location='comprarProducto.php'">Realizar una compra</button>
                                </div>
                                <div class="col d-grid gap-2">
                                    <button class="btn btn-secondary my-3" onclick="window.location='../Landing/index.php'">Regresar</button>
                                </div>
                            </div>
                            <table id="compras" class="table my-3" style="color:#fff;">
                                <thead>
                                    <tr>
                                        <th>Id</th>
                                        <th>Fecha</th>
                                ...
                                <tbody>
                                    <?php
                                        foreach($compras as $x => $x_value){
                                            echo "
                                            <tr>
                                                <td>".$x_value['idCompra']."</td>
                                                <td>".$x_value['fechaCompra']."</td>
                                                <td>".$x_value['totalCompra']."</td>
                                                <td>".$x_value['nombreProveedor']."</td>
                                                <td>
                                                    <form action='detallesCompra.php' method='POST'>
                                                        <input type='number' name='idCompra' value='".$x_value['idCompra']."'hidden>
                                                        <button class= 'btn btn-success' type='submit' style=' width: 100%; margin-bottom: 2%;'>Ver detalles</button>
                                                    </form>
                                                </td>
                                            </tr>
                                            ";
